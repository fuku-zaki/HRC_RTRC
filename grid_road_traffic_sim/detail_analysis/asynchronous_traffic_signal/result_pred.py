...
# %%
df_acc_old = pd.read_csv("result_all_old/df_acc.csv")
df_MC_old = pd.read_csv("result_all_old/df_MC.csv")

# %%
# asynchronous (20trial)
acc_values = df_acc.T.values.flatten()
mc_values  = df_MC.T.values.flatten()

# synnchronous (1trial)
acc_baseline = float(df_acc_old.iloc[6]["1"])
mc_baseline  = float(df_MC_old.iloc[6]["1"])

# %%
medianprops={'color': 'C1', 'linewidth':3, 'linestyle': '-.',}
meanprops={'markersize': 5, 'markeredgewidth': 2,}

plt.figure(figsize=(7,4), dpi=300, tight_layout=True)
plt.boxplot(acc_values, showmeans=True, medianprops=medianprops, meanprops=meanprops)
plt.axhline(float(acc_baseline), linestyle="--", linewidth=2, alpha=1, color="blue", label="Fully synchronized")
plt.xticks([1], [r"$\bar N_c=7$"])
plt.ylabel("RMSE", fontsize=20)
plt.tick_params(labelsize=20)
plt.legend(fontsize=15)
plt.savefig("phaseoffset_rmse.eps", dpi=300)
plt.show()

plt.figure(figsize=(7,4), dpi=300, tight_layout=True)
plt.boxplot(mc_values, showmeans=True, medianprops=medianprops, meanprops=meanprops)
plt.axhline(float(mc_baseline), linestyle="--", linewidth=2, alpha=1, color="blue", label="Fully synchronized")
plt.xticks([1], [r"$\bar N_c=7$"])
plt.ylabel("MC", fontsize=20)
plt.tick_params(labelsize=20)
plt.legend(fontsize=15)
plt.savefig("phaseoffset_mc.eps", dpi=300)
plt.show()
