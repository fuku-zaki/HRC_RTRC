...
class GridTraffics():
    ...
    def __init__(self, 
        ...
        low, high = -5, 5
        phase_range = np.arange(low, high + 1)
        if self.num_signals <= len(phase_range):
            self.phase_offset = np.random.choice(phase_range, size=self.num_signals, replace=False)
        else:
            print("Warning: number of signals exceeds unique phase range; allowing repeats.")
            self.phase_offset = np.random.randint(low, high + 1, size=self.num_signals)
    ...
    def simulate(self, n_simulation):
        ...
        simulate
        '''
        self.n_simultaion = n_simulation
        import time
    ...
    def _update_signals(self):
        ...
        for signal_idx in range(self.num_signals):
            ...
            # signal_state = self.random_step[self.now_step+int(10*signal_idx)]

            t_idx = self.now_step + int(self.phase_offset[signal_idx])
            t_idx = np.clip(t_idx, 0, len(self.random_step) - 1)
            self.signals_dict[signal_idx]['theta'].append(self.random_step[t_idx])
            signal_state = self.random_step[t_idx]
