...
class GridTraffics():
    ...
    def __init__(self, 
        ...
        """
        self.use_dynamic_turning = use_dynamic_turning
        # 交差点×接近方向ごとの基準確率（未指定なら等確率）
        # 例：{14:{'up':{'straight':0.65,'right':0.2,'left':0.15}, ...}, ...}
        self.turn_prob = turn_prob if turn_prob is not None else {}
    ...
    def _get_base_probs(self, sig_id, approach_dir):
        base = self.turn_prob.get(sig_id, {}).get(approach_dir, None)
        if base is None:
            return {'straight': 1/3, 'right': 1/3, 'left': 1/3}
        s = float(base.get('straight', 0.0))
        r = float(base.get('right',   0.0))
        l = float(base.get('left',    0.0))
        tot = s + r + l
        if tot <= 0:
            return {'straight': 1/3, 'right': 1/3, 'left': 1/3}
        return {'straight': s/tot, 'right': r/tot, 'left': l/tot}

    def _choose_next_direction(self, car_id, sig_id, approach_dir):
        # --- 1) Standard probability ---
        base = self._get_base_probs(sig_id, approach_dir)
        logits = {k: np.log(v + 1e-9) for k, v in base.items()}

        # --- 2) Corrected according to signal condition ---
        is_stop_UD = bool(self.signals_dict[sig_id]['judge'][-1])
        straight_is_UD = approach_dir in ('up', 'down')
        if is_stop_UD and straight_is_UD:
            logits['straight'] -= 10
        if (not is_stop_UD) and (not straight_is_UD):
            logits['straight'] -= 10

        # --- 3) Demand fluctuations ---
        t_now = getattr(self, 'now_step', 0) * self.dt
        logits['straight'] += 0.1 * np.sin(2*np.pi*t_now/3600)

        # --- 4) Local congestion penalty ---
        RL_in = self.signals_dict[sig_id]['RL_inflow_num'][-1]
        UD_in = self.signals_dict[sig_id]['UD_inflow_num'][-1]
        kappa = 0.01
        if straight_is_UD:
            logits['straight'] -= kappa * UD_in
        else:
            logits['straight'] -= kappa * RL_in

        # --- 5) No going straight outward at edge nodes ---
        H, W = self.num_grid
        r, c = divmod(sig_id, W)
        if (r == 0 and approach_dir == 'up') or \
        (r == H-1 and approach_dir == 'down') or \
        (c == 0 and approach_dir == 'left') or \
        (c == W-1 and approach_dir == 'right'):
            logits['straight'] = -1e9

        # --- 6) Prohibit directions with no adjacent nodes ---
        nb = self.signals_dict[sig_id]
        abs_map = {
        ...
        def has_neighbor(abs_dir):
            key = {'up':'up_sig_idx','down':'down_sig_idx',
                'right':'right_sig_idx','left':'left_sig_idx'}[abs_dir]
            return nb.get(key) is not None
        for rel in ('straight','right','left'):
            if not has_neighbor(abs_map[approach_dir][rel]):
                logits[rel] = -1e9

        # --- 7) softmax & sampling ---
        dirs = ['straight','right','left']
        p = self._softmax([logits[d] for d in dirs])
        choice = np.random.choice(dirs, p=p)
        next_abs = abs_map[approach_dir][choice]

        # --- 8) Do not allow U-turn ---
        opposite = {'up':'down','down':'up','right':'left','left':'right'}[approach_dir]
        if next_abs == opposite:
            next_abs = abs_map[approach_dir]['straight']
        return next_abs
        ...
        if not is_stop_UD: # if the upper and lower traffic lights are blue
            ...
            if current_d == 'up':
                ...
                if y<=next_border[1] and (y+dy)>next_border[1]: # if car is going over the stop line
                    ...
                    ## Determining new directions according to conditions (realistic routing)
                    if getattr(self, "use_dynamic_turning", False):
                        nd = self._choose_next_direction(
                            car_id=car_id,
                            sig_id=nearest_signal_idx,
                            approach_dir=current_d
                        )
                        opposite = {"up":"down","down":"up","right":"left","left":"right"}[current_d]
                        if nd == opposite:
                            nd = current_d  # 直進に矯正（または再サンプルでも可）
                        self.cars_dict[car_id]['next_d'] = nd
                    else:
                        self.cars_dict[car_id]['next_d'] = np.random.choice(
                            list(self.signals_dict[nearest_signal_idx]['directions']),
                            p=self.signals_dict[nearest_signal_idx]['trans_p']
                        )
